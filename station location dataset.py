...
for sta in ["CHY","HWA","ILA","KAU","TAP","TCU","TTN"]:
    ...
    tmp_info.columns=["location_code","station_location","county","district",\
                        "net","longitude","latitude","elevation","stamp code","address"]
    sta_filter=(tmp_info["location_code"].isin(station_info["location_code"]))
    add_df=tmp_info[~sta_filter][["location_code","latitude","longitude","elevation"]]
...
add_df=merged_station_code[~sta_filter][["TSMIP_code","Ins_longitude","Ins_latitude",\
                                        "Ins_elevation","TSMIP_short_code"]]
add_df.columns=["location_code","longitude","latitude","elevation (m)","station_code"]

save_index=[]
for sta_code in add_df["location_code"].unique():
    save_index.append(add_df[add_df["location_code"]==sta_code]["location_code"].index[-1])
