...
for EQ_ID in catalog.query("year==2016 & magnitude>=5.5")["EQ_ID"]:
    ...
    # fig.savefig(
    #     f"{path}/mag bigger 5.5 predict from model 510/{EQ_ID}_mag_{event['magnitude'].values[0]}_{mask_after_sec}sec PGA intensity Map.png", dpi=450, bbox_inches="tight"
    # )
    ...
    # fig.savefig(
    #     f"{path}/mag bigger 5.5 predict from model 510/{EQ_ID}_mag_{event['magnitude'].values[0]}_{mask_after_sec}sec true predict plot.png", dpi=450, bbox_inches="tight"
    # )
    try:
        fig, ax = warning_map(
            trace_info=event_prediction,
