...
data=multiple_station_dataset_new("D:/TEAM_TSMIP/data/TSMIP_new.hdf5",mode="test",
                                    mask_waveform_sec=mask_after_sec,test_year=2018,label_key=label,
...
for num in [1,2]:#[1,3,18,20]
    ...
    fig,ax=true_predicted(y_true=output_df["answer"],y_pred=output_df["predict"],
                    time=mask_after_sec,quantile=False,agg="point", point_size=12,target=label)
...
data2=pd.read_csv(f"predict/model2 {mask_after_sec} sec 1 triggered station prediction.csv")
...
output_df.to_csv(f"./predict/model1 2 {mask_after_sec} sec {trigger_station_threshold} triggered station prediction.csv",index=False)

fig.savefig(f"./predict/model1 2 {mask_after_sec} sec {trigger_station_threshold} triggered station.png")
