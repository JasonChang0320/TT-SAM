...
# traces = pd.merge(traces, catalog[["EQ_ID", "magnitude"]], how="left", on="EQ_ID")
# traces = traces.rename(columns={"epdis (km)": "epdis_km"})


# drop_traces = traces.query(f"pga<{np.log10(0.250)} & year!=2016").sample(
#     frac=0.4, random_state=0
# )
# under_sampled_trace = pd.concat([traces, drop_traces])
# under_sampled_trace.drop_duplicates(keep=False, inplace=True)
# traces = under_sampled_trace
# catalog = catalog[catalog["EQ_ID"].isin(traces["EQ_ID"].unique())]
