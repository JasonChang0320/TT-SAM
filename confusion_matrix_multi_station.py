...
PGA_threshold = np.log10(
    9.8 * np.array([0.01, 0.02, 0.025, 0.05, 0.08, 0.1])
)  # g*9.8 = m/s^2
performance_score = {
    "pga_threshold (g)": [],
    "confusion matrix": [],
...
for pga_threshold in PGA_threshold:
    ...
    performance_score["pga_threshold (g)"].append(
        np.round((10**pga_threshold) / 9.8, 3)
    )  # m/s^2 / 9.8 = g
    ...
    ax.set_title(
        f"EEW {mask_after_sec} sec confusion matrix, PGA threshold: {np.round((10**pga_threshold)/9.8,3)} (g)"
    )
    fig.savefig(
        f"{path}/confusion matrix/{mask_after_sec} sec pga_threshold {np.round((10**pga_threshold)/9.8,3)} g.png"
    )
    predict_table = pd.DataFrame(performance_score)
    predict_table.to_csv(
        f"{path}/confusion matrix/{mask_after_sec} sec confustion matrix table.csv",
        index=False,
    )
