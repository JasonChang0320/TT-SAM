...
class CNN(nn.Module):
    ...
    def __init__(self,
        ...
        self.lambda_layer_1 = LambdaLayer(lambda t: t / (torch.max(
            torch.max(torch.abs(t), dim=1, keepdim=True).values,
            dim=2,
            keepdim=True).values + self.eps))
        self.unsqueeze_layer1 = LambdaLayer(
            lambda t: torch.unsqueeze(t, dim=1))
        self.lambda_layer_2 = LambdaLayer(lambda t: torch.log(
            torch.max(torch.max(torch.abs(t), dim=1).values, dim=1).values +
            self.eps) / 100)
        self.unsqueeze_layer2 = LambdaLayer(
            lambda t: torch.unsqueeze(t, dim=1))
        self.conv2d1 = nn.Sequential(
            nn.Conv2d(1,
                      8,
                      kernel_size=(1, downsample),
                      stride=(1, downsample)),
...
class CNN_example(nn.Module):
    def __init__(self):
        ...
        # Convolution 2
        self.cnn2 = nn.Conv1d(in_channels=16, out_channels=8, kernel_size=1, stride=1, padding=0)
        self.relu2 = nn.ReLU() # activation
        # Max pool 2
        self.maxpool2 = nn.MaxPool1d(kernel_size=3)
        # Convolution 3
        self.cnn3 = nn.Conv1d(in_channels=8, out_channels=2, kernel_size=1, stride=1, padding=0)
...
class PositionEmbedding(nn.Module):  # paper page11 B.2
    ...
    def __init__(self,
        ...
        self.lat_coeff = 2 * np.pi * 1. / min_lat * (
            (min_lat / max_lat)**(np.arange(lat_dim) / lat_dim))
        self.lon_coeff = 2 * np.pi * 1. / min_lon * (
            (min_lon / max_lon)**(np.arange(lon_dim) / lon_dim))
        self.depth_coeff = 2 * np.pi * 1. / min_depth * (
        ...
        self.mask[depth_sin_mask] = 2 * lat_dim + 2 * lon_dim + np.arange(
            depth_dim)
        self.mask[
            depth_cos_mask] = 2 * lat_dim + 2 * lon_dim + depth_dim + np.arange(
                depth_dim)
        self.mask = self.mask.astype('int32')

    def forward(self, x):
        lat_base = x[:, :, 0:1].cuda() * torch.Tensor(
            self.lat_coeff).cuda()  # 這裡沒用到cuda!!
        lon_base = x[:, :, 1:2].cuda() * torch.Tensor(self.lon_coeff).cuda()
        depth_base = x[:, :, 2:3].cuda() * torch.Tensor(
            self.depth_coeff).cuda()
        ...
        index = (maskk.unsqueeze(0).unsqueeze(0)).expand(
            x.shape[0], 1, self.emb_dim).cuda()
...
class MDN(nn.Module):
    def __init__(self,input_shape=(150,),n_hidden=20, n_gaussians=5):
        ...
        self.z_h = nn.Sequential(
            nn.Linear(input_shape[0], n_hidden),
            nn.Tanh()
        )
...
class full_model(nn.Module):
    ...
    def forward(self,data):
        ...
        pga_pos_emb_output=self.model_Position(torch.DoubleTensor(data[2].reshape(-1,1,3)).float().cuda())
        pga_pos_emb_output=pga_pos_emb_output.reshape(-1,self.pga_targets,self.emb_dim)
        #data[2] 做一個padding mask [batchsize, PGA_target (15)], value: True, False (True: should mask)
        target_pad_mask=torch.ones_like(data[2],dtype=torch.bool) #避免 target position 在self-attention互相影響結果
...
class MixtureOutput(nn.Module):
    ...
    def forward(self, x):
        ...
        sigma = LambdaLayer(lambda x: x, self.eps)(
            sigma)  # Add epsilon to avoid division by 0
